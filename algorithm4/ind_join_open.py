...
class IndependentJoin(object):
    ...
    def generateSQL_DNF(self, separatorSubs=None):
        ...
        vlist = ["COALESCE(cast(q%d.pUse as text),cast('<%s>' as text))" % (i, l) for i, l in zip(counters, [c.lam for c in self.children])]
    ...
    def generateSQL_CNF(self, params):
        ...
        for tableAlias in tableAliases:
            if params['useLog']:
                if params['useNull']:
                    pSelect.append("COALESCE(cast(1-exp(%s.pUse) as text), '1')" % tableAlias)
                else:
                    pSelect.append(
