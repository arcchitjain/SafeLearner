...
class IndependentJoin(object):
    ...
    def getSafeQueryPlan(self, init=True):
        ...
        self.formattedUsedSeparators = [self.formatSeparatorVariable(sep) for sep in self.usedSeparatorVars]
    ...
    def generateSQL_DNF(self, separatorSubs=None):
        ...
        for (i, child) in enumerate(self.children):
            ...
            if child.hasGenericConstant():
                ...
                self.genericConstantStrs.append("q%d.%s" % (ident, genericConstantStr))
        ...
        if self.hasGenericConstant():
            selectAttributes.append("q%d.%s" % (genericConstantStrIdent, self.genericConstantStr))
            ...
            if previousIdent:
                ...
                if len(separatorSubs):
                    if ident in counterIdentToGenericConstantStr:
                        ...
                        for prevIdent in range(ident):
                            if prevIdent in counterIdentToGenericConstantStr:
                                conditions.append("q%d.%s = q%d.%s" % (ident, counterIdentToGenericConstantStr[ident], prevIdent, counterIdentToGenericConstantStr[prevIdent]))
                            conditions += ["q%d.c%d = q%d.c%d" % (prevIdent, i, ident, i) for (i, x) in separatorSubs if self.separatorInRelation1And2(i, prevIdent, ident, counterIdentToRelations)]
                        condition = "ON %s" % " and ".join(conditions)
                        subqueries.append("%s %s %s" % (joinType, newSubquery, condition))
                    else:
                        conditions = []
                        for prevIdent in range(ident):
                            conditions += ["q%d.c%d = q%d.c%d" % (prevIdent, i, ident, i) for (i, x) in separatorSubs if self.separatorInRelation1And2(i, prevIdent, ident, counterIdentToRelations)]
                    ...
                    if ident in counterIdentToGenericConstantStr:
                        ...
                        subqueries.append("%s %s ON TRUE" % (joinType, newSubquery))
        ...
        for (separatorReplacement, separatorVarsByComponent) in separatorSubs:
            ...
            for i in counters:
                ...
                if separatorInI:
                    ...
                    break
            selectAttributes.append("q%d.c%d" % (termIdentWithThisSubstitution, separatorReplacement))

        # pString = '*'.join(["COALESCE(q%d.pUse,q%d.lam)" % (i, i) for i in counters])
        vlist = ["COALESCE(q%d.pUse, <%s>)" % (i, l) for i, l in zip(counters, [c.lam for c in self.children])]
        pString = reduce(lambda x,y: "l1prod_n(" + x + "," + y + ")", vlist)
        ...
        sql = "\n select %s as pUse from %s" % (selectString, " ".join(subqueries))
        return sql
    ...
    def generateSQL_CNF(self, params):
        ...
        for child in self.children:
            ...
            for genericIdentifier in childGenericIdentifiers:
                if genericIdentifier in genericIdentifierToTableAliasMap:
                    ...
                    genericIdentifierToTableAliasMap[genericIdentifier] = set([tableAlias])
            ...
            for usedSeparatorVariable in self.usedSeparatorVars:
                if child.usesSeparator(usedSeparatorVariable):
                    ...
                    if formattedSeparator in usedSeparatorToTableAliasMap:
                        ...
                        usedSeparatorToTableAliasMap[formattedSeparator] = set([tableAlias])
            ...
            tableAliasToVariablesMap[tableAlias] = tableAliasToUsedSeparatorsMap[tableAlias].union(childGenericIdentifiers)

        if not tableAliasesUsingAllSeparators:
            raise Exception("No subquery containing all separators!")

        for tableAlias in tableAliases:
            tableAliasToMissingGenericIdentifiersMap[tableAlias] = self.genericIdentifiers.difference(tableAliasToGenericIdentifiersMap[tableAlias])
            tableAliasToMissingSeparatorsMap[tableAlias] = set(self.formattedUsedSeparators).difference(tableAliasToUsedSeparatorsMap[tableAlias])
        ...
        for tableAlias in tableAliases:
            if tableAliasToMissingGenericIdentifiersMap[tableAlias]:
                ...
                withClauses.append("%s as (%s)" % (aliasForMissingGenericVarsTable, tableAliasToSubquerySQLMap[tableAlias]))
                index = 1
                extraDomainSelectVars = []
                extraDomainTables = []
                for missingGenericVariable in tableAliasToMissingGenericIdentifiersMap[tableAlias]:
                    extraDomainAlias = "A_%d" % index
                    extraDomainSelectVars.append("%s.v0 as %s" % (extraDomainAlias, missingGenericVariable))
                ...
                extraDomainFromClause = "%s, %s" % (aliasForMissingGenericVarsTable, ", ".join(extraDomainTables))
                withClauses.append("%s as (select %s from %s)" % (tableAlias, extraDomainSelectClause, extraDomainFromClause))
            else:
                withClauses.append("%s as (%s)" % (tableAlias, tableAliasToSubquerySQLMap[tableAlias]))
                ...
                for variable in tableAliasToVariablesMap[tableAlias]:
                    for previousAlias in previousAliases:
                        if variable in tableAliasToVariablesMap[previousAlias]:
                            joinConditions.append("%s.%s = %s.%s" % (tableAlias, variable, previousAlias, variable))
                            break
                if joinConditions:
                    joinConditionsString = " AND ".join(joinConditions)
                else:
                    joinConditionsString = "TRUE"
                joinSubqueries.append(" INNER JOIN %s ON %s" % (tableAlias, joinConditionsString))
            ...
            if not previousAliases:
                ...
                for variable in tableAliasToVariablesMap[tableAlias]:
                    ...
                    if trueOnMissingJoinAlias:
                        joinConditions.append("%s.%s = %s.%s" % (tableAlias, variable, trueOnMissingJoinAlias, variable))
                    elif falseOnMissingJoinAliases:
                        falseOnMissingJoinAliasesString = ", ".join(["%s.%s" % (alias, variable)for alias in falseOnMissingJoinAliases])
                        joinConditions.append("%s.%s = COALESCE(%s)" % (tableAlias, variable, falseOnMissingJoinAliasesString))
                if joinConditions:
                    joinConditionsString = " AND ".join(joinConditions)
                else:
                    joinConditionsString = "TRUE"
                joinSubqueries.append(" %s JOIN %s ON %s" % (joinType, tableAlias, joinConditionsString))
            previousAliases.append(tableAlias)

        selectVariables = set(self.formattedUsedSeparators).union(self.genericIdentifiers)
        selectClause = []
        for variable in selectVariables:
            ...
            if trueOnMissingTableAlias:
                ...
                falseOnMissingTableAliasesString = ", ".join(["%s.%s" % (alias, variable) for alias in falseOnMissingTableAliases])
                selectClause.append("COALESCE(%s) as %s" % (falseOnMissingTableAliasesString, variable))
        ...
        for tableAlias in tableAliases:
            if params['useLog']:
                if params['useNull']:
                    ...
                    pSelect.append("COALESCE(CASE WHEN %s.pUse != '-Infinity' THEN 1-exp(%s.pUse) ELSE 1 END, 1)" % (tableAlias, tableAlias))
            else:
                pSelect.append("COALESCE(1-%s.pUse, 1)" % tableAlias)
        pUseString = "1 - ( %s )" % (' * '.join(pSelect))
        if params['useLog']:
            if params['useNull']:
                selectClause.append("CASE WHEN %s > 0 THEN ln(%s) ELSE NULL END AS pUse" % (pUseString, pUseString))
            else:
                selectClause.append("CASE WHEN %s > 0 THEN ln(%s) ELSE '-Infinity' END AS pUse" % (pUseString, pUseString))
        else:
        ...
        withString = ",\n".join(withClauses)
        joinSQL = "\nWITH %s\nselect %s from %s" % (withString, selectString, fromString)
    ...
    def __repr__(self):
        return "Independent Join (true on missing = %s): %s " % (self.trueOnMissing, ", ".join([x.__repr__() for x in self.children]))
