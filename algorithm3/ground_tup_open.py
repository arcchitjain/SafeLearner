        ...
        if rel.getConstraints():
            ...
            for constant in rel.getConstraints():
                if (not constant or
                        ...
                        or constant == '-c'):
                    selectAttributes.append("<%s>.v%d as c%d" % (relSym, tablePos, relArgs[argPos].getReplacement()))
                    argPos += 1
                tablePos += 1
        else:
            for i in range(len(relArgs)):
                if rel.isVariable(i):
                    ...
                    selectAttributes.append("<%s>.v%d as c%d" % (relSym, i, relArgs[i].getReplacement()))
