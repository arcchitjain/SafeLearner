...
class IndependentProject(object):
    ...
    def generateSQL_DNF(self, separatorSubs=None):
        ...
        if len(groupBy):
            ...
            groupByString = ''
        selectString = ', '.join(groupBy + ['ior_n(COALESCE(pUse,0))'])
        separatorSubs.append((self.replacementVal, self.separator))

        childSQL = self.child.generateSQL_DNF(separatorSubs[:])
        sql = "\n -- independent project \n select %s as pUse from (%s) as q%d %s " % (selectString, childSQL, algorithm.counter(), groupByString)
        ...
        if self.child.hasGenericConstant():
            ...
            groupByString = 'group by ' + ', '.join(groupBy)
            sql = "\n -- independent project \n select %s, %s as pUse from (%s) as q%d %s " % (genericConstantStr, selectString, childSQL, algorithm.counter(), groupByString)
        else:
            sql = "\n -- independent project \n select %s as pUse from (%s) as q%d %s " % (selectString, childSQL, algorithm.counter(), groupByString)
