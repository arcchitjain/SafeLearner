...
class ProbFOIL(LearnEntail):
    ...
    def __init__(self, data, beam_size=5, logger='probfoil', minhc = 0.00001, minpca = 0.00001, lr1 = 0.001, lr2 = 0.0001, iterations = 10000, ssh = False, maxAmieRules = None, cwLearning = False, quotes = False, m=1, cost=1, l=None, p=None, disableTypeConstraints = False, closed_world=False, global_score = 'cross_entropy', optimization_method = 'incremental', candidate_rules = 'amie', **kwargs):
    ...
    def learn_getQueryList(self, hypothesis):
            ...
            while k < len(hypothesis.queryString):
                ...
                if hypothesis.queryString[k-1]=="(":
                    ...
                    if ord(hypothesis.queryString[k]) <= 64 + self.targetArity:
                        ...
                        value = self.examples[i][baseNumber]
                    ...
                    if replacePredicate == True: #This means that the literal contains at least 1 fixed variable that needs instantiation
                        ...
                        if reallyReplacePredicate == True: #Eg: '(A,B,C)' ==> '(C)'
                            instantiatedTableSet.add(newTable)
                ...
                if k == len(hypothesis.queryString) - 1:
                    ...
                    if '' not in clauseList:
                        ...
                        self.querySS[i] = instantiatedQueryString
                    else:
                        self.querySS[i] = ''
                k = k + 1
            i = i + 1
        ...
        for tableItem in instantiatedTableSet - self.previousInstantiatedTableSet:                
            ...
            if selectString == "" or whereString == "":
    ...
    def learn(self):
        ...
        while True:
            ...
            lastProb = next_hypothesis.get_rule_probability()
            if lastProb == None or self.learnAllRules:
                lastProb = 1
            if next_hypothesis.gscore > current_score and lastProb > 6*self.tolerance:
                hypothesis = next_hypothesis
                current_score = next_hypothesis.gscore
                if self.open_world:
                    self.lams = newLam
                    
                getLogger('probfoil').info('Rule ACCEPTED in hypothesis')
                
            else:
                if not self.lastRuleMerged:
                    rule = hypothesis
                    while rule.previous is not None and hasattr(rule, "oldProb"):
                        rule.max_x = rule.oldProb
                        rule.scores = rule.oldScores
                        self._select_rule(rule)
                        rule = rule.previous
                   
                getLogger('probfoil').info('Rule REJECTED from hypothesis')
                
                if self.candidate_rules == "amie":
                    failedAttempts += 1
                    if len(hypothesis.to_clauses()) > 6 or failedAttempts > 5 or len(self.selectedAmieRules) == len(self.AmieRuleList):
                        break
                else:
                    break
        ...
        hypothesis = self.learn_pruneHypothesis(hypothesis)
        ...
        if not(hasattr(hypothesis, 'weightedAccuracy')) or hypothesis.weightedAccuracy == "":
            hypothesis.weightedAccuracy = self.getGlobalScore_again(hypothesis, self.lams, scoreType = "accuracy")
        if not(hasattr(hypothesis, 'crossEntropy')) or hypothesis.crossEntropy == "":
            hypothesis.crossEntropy = self.getGlobalScore_again(hypothesis, self.lams, scoreType = "cross_entropy")
        if not(hasattr(hypothesis, 'squaredLoss')) or hypothesis.squaredLoss == "":
