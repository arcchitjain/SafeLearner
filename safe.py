...
class SafeSample(object):
    ...
    def prepareQuery(self, relationsToSample, relsObjects, querySQL):
        ...
        for rel in relsObjects:
            ...
            # more than just v0
            sampledTables.append("%s as (select v0, CASE WHEN random() < p THEN 1 ELSE 0 END as p from %s)" % (relSym, relSym))

        safeSampleQuery = "with %s %s" % (', '.join(sampledTables),querySQL)
    ...
    def safeSample(self, relationsToSample, relsObjects, querySQL, numSamples, epsilon=0, delta=0):
        safeSampleQuery = self.prepareQuery(relationsToSample, relsObjects, querySQL)
        self.sampleRecord = []
        self.estimatesRecord = []
    ...
    def sampleOptimal(self, safeSampleQuery, numSamples, epsilon, delta):
        ...
        try:
            ...
            # step 1
            (muHatZ, sampledZ) = self.stoppingRuleAlgorithm(np.amin([0.5, math.sqrt(epsilon)]), delta / 3.0, safeSampleQuery)
