...
class LearnEntail(object):
    ...
    def transformFreeVars(self, rule, ascii):
        ...
        while k < len(ruleString):
            if ruleString[k - 1] == "(" or (
                ruleString[k - 1] == "," and ruleString[k - 2] != ")"
            ):
                if ord(ruleString[k]) <= 64 + self.targetArity:
                    newRuleString = newRuleString + ruleString[k]
                else:
                    newRuleString = newRuleString + chr(
                        ord(ruleString[k]) - 64 - self.targetArity + ascii
                    )
            else:
                newRuleString = newRuleString + ruleString[k]
            k = k + 1
        if ruleString != newRuleString:
            getLogger(self._logger).debug(
                "transformFreeVars: Changing %s to %s" % (ruleString, newRuleString)
            )
    ...
    def incrementFreeVars(self, rule, step):
        ...
        while k < len(ruleString):
            if ruleString[k - 1] == "(" or (
                ...
                if ord(ruleString[k]) <= 64 + self.targetArity:
                    ...
                    getLogger(self._logger).error(
                        "incrementFreeVars: Error changing %s to %s"
        ...
        if ruleString != newRuleString:
            getLogger(self._logger).debug(
                "incrementFreeVars: Changing %s to %s" % (ruleString, newRuleString)
            )
        return newRuleString
                ...
                if query[i] != ",":
                    ...
                    if "V" + str(variableList.index(var)) not in variableMapping:
                        if actualTable == "p":
                            ...
                            variableMapping[
                                "V" + str(variableList.index(var))
                            ] = self.predicateDict[actualTable][allIndices[argCount]]
    ...
    def executeCanonicalSQLQuery(self, SQLQuery, tableList, variableMapping):
                ...
                if expression[0:5] == "table":
                    ...
                    while j < len(expression) - 1:
                        if expression[j : j + 2] == "z_":  # Lambda identified
                            substitutedExpression = (
                                substitutedExpression + expression[lastEnd:j]
                            )
                            start = j
                            j += 2
                            while (
                            ...
                            if actualTable == "p":
                                ...
                                substitutedExpression = substitutedExpression + str(
                                    self.lams[table]
                                )
                            else:
                                substitutedExpression = substitutedExpression + str(
                                    self.lams[actualTable]
                                )
                            lastEnd = j
                            continue
                    ...
                    with errstate(divide="ignore", invalid="ignore"):
                        value = evaluate_expression(substitutedExpression)
                    if str(value) != "-inf":
        ...
        try:
            ...
            getLogger(self._logger).log(
                8, "Execute SQL >> SQL error \t: " + e.pgerror[:-1].replace("\n", "\t")
            )
            getLogger(self._logger).warning(
                "Execute SQL >> Uninstantiated Query \t: " + SQLQuery
            )
            getLogger(self._logger).warning(
                "Execute SQL >> Instantiated Query \t: " + trueSQLQuery
            )
    ...
    def getConjunctProbability(self, query):
        ...
        prob = self.executeCanonicalSQLQuery(
            canonicalSQLQuery, tableList, variableMapping
        )
        return prob
    ...
    def checkIsomorphicRule(self, rule):
            ...
            while rule[i] != ")":
                if rule[i] != ",":
                    if ord(rule[i]) < 65 + self.targetArity:
                        ...
                        canonicalRule = canonicalRule + chr(
                            65 + self.targetArity + variableList.index(rule[i])
                        )
                else:
        ...
        if newCanonicalRule in self.canonicalRuleList:
            ...
            getLogger(self._logger).log(
                9, "Canonical Rule\t\t\t\t\t: " + newCanonicalRule
            )
        else:
                ...
                for i, example in zip(to_eval, examples):
                    ...
                    while j < len(bodyString):
                        ...
                        while bodyString[j] != ")":
                            if bodyString[j] != ",":
                                if ord(bodyString[j]) <= 64 + self.targetArity:
                                    ...
                                    base = self.predicateDict[self.targetPredicate][
                                        baseNumber
                                    ]
                                    ...
                                    newTable = (
                                        newTable
                                        + "_"
                                        + str(self.universalConstantId[value])
                                    )
                                    whereList.append(
                                        (l, self.universalConstantId[value])
                                    )
                                else:
                        ...
                        if (
                            ...
                            if instantiatedQueryString == "":
                                if negation == False:
                                    instantiatedQueryString = bodyString[
                                        start : endRound + 1
                                    ]
                                else:
                                    instantiatedQueryString = (
                                        "~" + bodyString[start : endRound + 1]
                                    )
                            else:
                                if negation == False:
                                    instantiatedQueryString = (
                    ...
                    if instantiatedQueryString == "":
                        ...
                        if self.querySS[i] != "":
                            ...
                            newQueryString = self.incrementFreeVars(
                                instantiatedQueryString, hypothesisFreeVars
                            )
                    ...
                    if prob in ["Failed to parse", None, "Query is unsafe"]:
                        getLogger(self._logger).log(
                            8,
                            "%d.\t%s\t: %s\t= %s"
                            % (i, fullString, str(example), str(prob)),
                        )
                        scores_predict = rule.previous.scores
                        ...
                        scores_predict[
                            ...
                            prob
                        )  # Not Rounding to 12 decimal places to avoid float precision error
                        ...
                        getLogger(self._logger).log(
                            7,
                            "%s\t: %d.\t%s\t= %s"
                            % (fullString, i, str(example), str(scores_predict[i])),
                        )
            ...
            for i, example in enumerate(self.examples):
                ...
                if prob in ["Failed to parse", None, "Query is unsafe"]:
                    getLogger(self._logger).log(
                        8,
                        "%d.\t%s\t: %s\t= %s"
                        % (i, self.querySS[i], str(example), str(prob)),
                    )
                    ...
                    getLogger(self._logger).log(
                        7,
                        "%s\t: %d.\t%s\t= %s"
                        % (self.querySS[i], i, str(example), str(scores_predict[i])),
                    )
