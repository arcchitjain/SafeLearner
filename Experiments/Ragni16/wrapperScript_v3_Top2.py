...
def matchAccuracy(fold, person, ruleDir, precedence):
	...
	def getSecondMax(possibleResponses, responseNumList):
		count = 0
		m1 = m2 = float('-inf')
		i1 = i2 = 0
		response2 = ''
		for i, x in enumerate(responseNumList):
			count += 1
			if x > m2:
				if x >= m1:
					m1, m2 = x, m1
					...
					m2 = x
					i2 = i
			response2 = possibleResponses[i2]
		return response2 if count >= 2 else None
	...
	for id in testSyllogisms:
		...
		secondTestPredictions[id] = getSecondMax(possibleResponses, responseNumList)

		#print('Person ' + str(person) + '\t Syllogism ' + str(id) + '\t Predicted ' + testPredictions[id] + '\t Actual ' + testAnswers[id])
	
		if testPredictions[id] == testAnswers[id] or secondTestPredictions[id] == testAnswers[id]:
			match += 1
		total += 1
		tp_person += tp_id
		tn_person += tn_id
