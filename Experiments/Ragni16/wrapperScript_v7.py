...
weight = [float(item) for item in sys.argv[6].split(' ')]
if len(weight) != 4:
	print('Error: Length of weight is not 4')
...
def breakQuery(query, person, id, response):
	queryList = query.split('\n')
	newQueryList = []
	for r in responses[:-1]:
		newQueryList.append('0::' + r[1:] + str(person) + '(a,b).')

	newQueryList.append('0::' + response + str(person) + '(a' + str(id) + ',c' + str(id) + ').')
	newQueryList.append('0::' + response + str(person) + '(c' + str(id) + ',a' + str(id) + ').')
	
	for i, line in enumerate(queryList):
		if line == '':									# Ignore Emtpy lines
			continue

		if 'A' in line and 'C' in line: 				# Rule
			if response in line:
				newQueryList.append(line)
		elif 'query' in line:							# Query Statements
			if str(id) + ',' in line and response in line:
				newQueryList.append(line)
		else:											# Probabilistic Facts
			if str(id) + ',' in line:
				newQueryList.append(line)

	newQuery = '\n'.join(newQueryList)
	#print('Problog Query for id ' + str(id) + ' and response ' + str(response) + ':\n' + newQuery + '\n')
	return newQuery
...
def matchAccuracy(fold, person, ruleDir, precedence):
		...
		if learn == True:
			...
			for rule in pfRules:
				if rulesIndicator[0] == '2':
					prob = float(pfPrecision)*weight[0]/sumWeight
					if prob > 0:
						rules.append(str(prob) + '::' + rule[:-1]+'.')
	...
	if rulesIndicator[2] == '0':
		...
		for i, rule in enumerate(logicallyValidRules):
			rule = rule.replace('#',str(person)).replace(' ','')
			l = rule.split(':')
			old_prob = float(l[0])
			new_prob = old_prob * weight[2]/sumWeight
			l[0] = str(new_prob)
			if new_prob > 0:
				logicallyValidRules[i] = ':'.join(l)
			else:
				logicallyValidRules[i] = ''
	...
	for response in responses:
		...
		for rule in hypothesis:
			...
			if rulesIndicator[1] == '2':
				prob = float(precision)*weight[1]/sumWeight
				if prob > 0:
					personIndependentRules.append(str(prob) + '::' + transformedRule +'.')
	...
	for id in testSyllogisms:
		...
		for response, freq in zip(responseList, freqList):
			...
			new_prob = prob * weight[3] /sumWeight
			if new_prob > 0:
				facts.append(str(new_prob) + '::' + atom)
	...
	for id in testSyllogisms:	
		...
		for response in responses:
			newProbLogQuery = breakQuery(problogQuery, person, id, response)
			p = PrologString(newProbLogQuery)
			inferences = get_evaluatable().create_from(p).evaluate()
			
			# Convert every term in the key to a string
			for term, probability in inferences.items():
				inferenceDict[str(term)] = probability
...
for precedence in precedences:
	...
	print(str(rulesIndicator) + ': Net Accuracy for precedence ' + str(precedence) + ' and weight ' + str(weight) + '\t: ' + str(sum(accuracyList)/len(accuracyList)))
