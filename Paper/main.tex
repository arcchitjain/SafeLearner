...
Choosing $\mathrm{P}_\mathcal{D}(t) = 0$ for tuples missing from PDB $\mathcal{D}$ is the probabilistic version of the CWA. Furthermore, the probability of any Boolean query $Q$ w.r.t. a PDB $\mathcal{D}$ is
...
Lastly, we perform Stochastic Gradient Descent (SGD) to optimize the rule weights. %In order to get the gradients, we use automatic differentiation \cite{baydin2017automatic} on the predicted probability function with respect to $p_h$, for a series of randomly selected examples.
...
%\State Initialize $\lambda$ for every relation, including $target$ 
...
%\State Sample closed-world $target$ tuples in $E_{cw}$ and compute $w_{cw}$ \label{alg_line:samp1}
%\State Sample open-world $target$ tuples in $E_{ow}$ and compute $w_{ow}$ \label{alg_line:samp2}
...
\FORALL{$i$ in range(0, MaxIterations)} \label{alg_line:SGD1}
...
\begin{figure}[H]
\begin{center}
    \scalebox{0.30}{\includegraphics{WithNoiseCW/Exp1_CE_v3.pdf}}
    \caption{\algorithmname has better Cross Entropy}\ \\
    \begin{tabular}{@{}c@{}c@{}c@{}}
    ...
    \end{tabular}
    \caption{Precision-Recall Curves for 5 relations in NELL Sports Dataset} %\guy{fonts are very small: split in two columns?}
\end{center}
\end{figure}
\iffalse % Begin Block Comment 
\begin{figure}[H]
\begin{center}
    \scalebox{0.33}{\includegraphics{WithNoiseCW/Exp2_CE_v3.pdf}}
    \caption{\algorithmname has better Cross Entropy in Task 1}\ \\
    %\makegapedcells
    \begin{tabular}{@{}c@{}c@{}c@{}}
        \scalebox{.33}{\includegraphics{WithNoiseCW/Exp2_apft_det.pdf}} & \scalebox{.33}{\includegraphics{WithNoiseCW/Exp2_aps_det.pdf}} & \scalebox{.33}{\includegraphics{WithNoiseCW/Exp2_apil_det.pdf}} \\
        \scalebox{0.8}{\includegraphics{WithNoiseCW/legend.pdf}} & \scalebox{.33}{\includegraphics{WithNoiseCW/Exp2_tps_det.pdf}} & \scalebox{.33}{\includegraphics{WithNoiseCW/Exp2_tpil_det.pdf}}
    \end{tabular}
    \caption{Precision-Recall Curves for Task 2}
\end{center}
\end{figure}
\fi % End Block Comment
...
Learning Time = 1 hour 17 minutes \\ %4601 seconds      
...
Learning Time = 6 hours 42 minutes \\ %24134 seconds
...
Learning Time = 4 hours 54 minutes \\ %17635 seconds
...
Learning Time = 18 hours 23 minutes \\ %66208 seconds
...
Learning Time = 5 hours 51 minutes \\ %21117 seconds
...
Learning time = 4 hours 1 minutes \\ %14478 seconds
