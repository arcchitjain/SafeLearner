...
class IndependentJoin(object):
    ...
    def getSafeQueryPlan(self, init=True):
        ...
        if init:
            ...
            self.lam = reduce(
                lambda x, y: ln(E ** (x) + E ** (y) - E ** (x + y)),
                [x.lam for x in self.children],
            )
        else:
            self.children = []
                ...
                if len(separatorSubs):
                    if ident in counterIdentToGenericConstantStr:
                        ...
                        for prevIdent in range(ident):
                            ...
                            conditions += [
                                "q%d.c%d = q%d.c%d" % (prevIdent, i, ident, i)
                                for (i, x) in separatorSubs
                                if self.separatorInRelation1And2(
                                    i, prevIdent, ident, counterIdentToRelations
                                )
                            ]
        ...
        for (separatorReplacement, separatorVarsByComponent) in separatorSubs:
            ...
            for i in counters:
                ...
                if separatorInI:
                    ...
                    break
            selectAttributes.append(
                "q%d.c%d" % (termIdentWithThisSubstitution, separatorReplacement)
            )

        # pString = '*'.join(["COALESCE(q%d.pUse,q%d.lam)" % (i, i) for i in counters])
        vlist = [
            "COALESCE(cast(q%d.pUse as text),cast('<%s>' as text))" % (i, l)
    ...
    def generateSQL_CNF(self, params):
        ...
        if params["useLog"]:
            if params["useNull"]:
                ...
                pUseTemplate = (
                    "CASE WHEN %s.pUse != '-Infinity' THEN 1-exp(%s.pUse) ELSE 1 END"
                )
        else:
        ...
        for tableAlias in tableAliases:
            if params["useLog"]:
                if params["useNull"]:
                    pSelect.append(
                        "COALESCE(cast(1-exp(%s.pUse) as text), '1')" % tableAlias
                    )
                else:
                    pSelect.append(
                        "COALESCE(CASE WHEN %s.pUse != '-Infinity' THEN cast(1-exp(%s.pUse) as text) ELSE '1' END, '1')"
                        % (tableAlias, tableAlias)
                    )
            else:
                pSelect.append("COALESCE(cast(1-%s.pUse as text), '1')" % tableAlias)
    ...
    def separatorInRelation1And2(self, sepId, rel1Id, rel2Id, counterIdentToRelations):
        separatorInR1 = False
        if (
            rel1Id not in counterIdentToRelations
            or rel2Id not in counterIdentToRelations
        ):
            return False
